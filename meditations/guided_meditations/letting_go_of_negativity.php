...
// Calculate XP for the current level
        $xpGain = max($initialXP - ($currentLevel * $xpDecayRate), 500);

// You can add a check here to ensure that XP never drops below 500:
        if ($xpGain < 500) {
            $xpGain = 500; // Ensures that XP will always be 500 or greater
        }

        // Calculate Blossoms gain (can remain unchanged or adjusted separately)
        $blossomsGain = ($currentLevel <= 2) ? 10 : 5;
...
<style>
    body {
        ...
        background-color: hotpink; /* Start with black */
    ...
    @keyframes fadeToImage {
        ...
        100% { background: url('../../pictures/letting_go_of_negativity_background.jpg') no-repeat center center / cover; }
    }
...
<div class="text">Letting Go of Negativity</div>

<div id="meditationContent"></div>

<!-- Audio Element -->
<audio id="meditationMusic" src="../../audio/letting_go_of_negativity_music.mp3" preload="auto" loop></audio>

<!-- Dialog -->
<audio id="meditationDialog" src="../../audio/letting_go_of_negativity_dialog.mp3" preload="auto"></audio>
...
<div id="imageContainer" class="image-container" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; z-index:1000;">
    <img id="lettingGoOfNegativityImage" alt="Letting Go of Negativity Image" style="width:100%; height:100%; object-fit:cover; opacity: 0; transition: opacity 2s;">
</div>
...
<div class="description">
    <img src="../../pictures/letting_go_of_negativity.jpg" alt="Letting Go of Negativity" class="intro-image">
    <p></p>
    <p></p>
    <button class="start-button" id="startButton" style="font-family: 'Nunito', sans-serif; background-color:hotpink">Start Meditation</button>
    <button id="pauseButton" style="display: none; opacity: 0; position: fixed; left: 50%; top: 50%; transform: translate(-50%, -50%); background: hotpink; border: none; padding: 20px; border-radius: 10px; cursor: pointer; transition: opacity 0.5s;">
    ...
    <div id="progressContainer" style="display: none; position: fixed; left: 50%; top: 55%; transform: translateX(-50%); width: 80%; height: 50px; background-color: #ddd; border-radius: 5px; cursor: pointer;">
        <div id="progressBar" style="height: 100%; background-color: hotpink; width: 0%; border-radius: 5px; transition: width 0.2s ease-in-out;"></div>
    </div>
        ...
        <div class="modal-content">
            ...
            <h3>You’ve earned <strong>
                <span class="blossoms-number">
                    <?php echo ($currentLevel <= 2) ? 10 : 5; ?>
                </span>
                </strong> Blossoms.</h3>
            <h3>You’ve also gained <strong>
            <span class="xp-number">
                <?php echo max(3500 - ($currentLevel * 500), 500); ?>
            </span>
                </strong> XP.</h3>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const lettingGoOfNegativityImage = document.getElementById("lettingGoOfNegativityImage");
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function updateBlossomsAndXP() {
            fetch('letting_go_of_negativity.php', {
                method: 'POST',
                headers: {
