...
if ($userId) {
    ...
    if (isset($data['modal_shown']) && $data['modal_shown'] === 'true') {
        // Calculate XP based on the user's level
        $initialXP = 3000;
        $xpDecayRate = 500; // Amount to reduce per level

// Calculate XP for the current level
        $xpGain = max($initialXP - ($currentLevel * $xpDecayRate), 350);

// You can add a check here to ensure that XP never drops below 500:
        if ($xpGain < 350) {
            $xpGain = 350; // Ensures that XP will always be 500 or greater
        }

        // Calculate Blossoms gain (can remain unchanged or adjusted separately)
        $blossomsGain = ($currentLevel <= 9) ? 10 : 5;
...
<head>
    ...
    <title>Mindful Presence Meditation</title>
...
<style>
    body {
        ...
        background: yellow; /* Start with black */
    ...
    @keyframes fadeToImage {
        0% { background: yellow; }
        100% { background: url('../../pictures/mindful_presence_background_meditation.jpg') no-repeat center center / cover; }
    }
...
<div class="text" style="color: black;">Mindful Presence Meditation</div>

<div id="meditationContent"></div>

<!-- Audio Element -->
<audio id="meditationMusic" src="../../audio/mindful_presence_meditation_music.mp3" preload="auto" loop></audio>

<!-- Dialog -->
<audio id="meditationDialog" src="../../audio/mindful_presence_meditation_dialog.mp3" preload="auto"></audio>
...
<div id="imageContainer" class="image-container" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; z-index:1000;">
    <img id="mindfulPresenceMeditationImage" alt="Mindful Presence Meditation Image" style="width:100%; height:100%; object-fit:cover; opacity: 0; transition: opacity 2s;">
</div>
...
<div class="description">
    <img src="../../pictures/mindful_presence_meditation_intro.jpg" alt="Mindful Presence Image"" class="intro-image">
    <p></p>
    <p></p>
    <button class="start-button" id="startButton" style="font-family: 'Nunito', sans-serif; background: yellow; color: black; border: none; padding: 10px 20px; border-radius: 5px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.2); transition: all 0.3s ease;">
        Start Meditation
    </button>
    <button id="pauseButton" style="display: none; opacity: 0; position: fixed; left: 50%; top: 50%; transform: translate(-50%, -50%); background: yellow; border: none; padding: 20px; border-radius: 10px; cursor: pointer; transition: opacity 0.5s;">
        <i id="pauseIcon" class="fas fa-pause" style="font-size: 40px; color: black;"></i>
    </button>
    <div id="progressContainer" style="display: none; position: fixed; left: 50%; top: 55%; transform: translateX(-50%); width: 80%; height: 50px; background-color: #ddd; border-radius: 5px; cursor: pointer;">
        <div id="progressBar" style="height: 100%; background: yellow; width: 0%; border-radius: 5px; transition: width 0.2s ease-in-out;"></div>
    </div>
    <div id="countdownContainer">
        <span id="countdownTimer">02:57</span>
    </div>
        ...
        <div class="modal-content">
            ...
            <h3>You’ve earned <strong>
                <span class="blossoms-number">
                    <?php echo ($currentLevel <= 7) ? 10 : 5; ?>
                </span>
                </strong> Blossoms.</h3>
            <h3>You’ve also gained <strong>
            <span class="xp-number">
                <?php echo max(3000 - ($currentLevel * 500), 350); ?>
            </span>
                </strong> XP.</h3>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const mindfulPresenceMeditationImage = document.getElementById("mindfulPresenceMeditationImage");
...
<script>
    document.getElementById("startButton").addEventListener("click", function () {
        setTimeout(() => {
            let audio = new Audio("../../mindful_presence_meditation_music.mp3");
            audio.play();
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function updateBlossomsAndXP() {
            fetch('mindful_presence_meditation.php', {
                method: 'POST',
                headers: {
