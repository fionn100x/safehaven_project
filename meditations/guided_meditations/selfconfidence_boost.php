...
if ($userId) {
    ...
    if (isset($data['modal_shown']) && $data['modal_shown'] === 'true') {
        ...
        $initialXP = 4000;
        $xpDecayRate = 1000; // Amount to reduce per level
...
<style>
    body {
        ...
        background-color: darkgreen; /* Start with black */
    ...
    @keyframes fadeToImage {
        ...
        100% { background: url('../../pictures/self_confidence_boost_background.jpg') no-repeat center center / cover; }
    }
...
<div class="text">Self-Confidence Boost</div>

<div id="meditationContent"></div>

<!-- Audio Element -->
<audio id="meditationMusic" src="../../audio/self_confidence_boost_music.mp3" preload="auto" loop></audio>

<!-- Dialog -->
<audio id="meditationDialog" src="../../audio/self_confidence_boost_dialog.mp3" preload="auto"></audio>
...
<div id="imageContainer" class="image-container" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; z-index:1000;">
    <img id="selfConfidenceImage" alt="Self-Confidence Image" style="width:100%; height:100%; object-fit:cover; opacity: 0; transition: opacity 2s;">
</div>
...
<div class="description">
    <img src="../../pictures/self_confidence_boost_intro.jpg" alt="Self-Confidence Boost" class="intro-image">
    <p></p>
    <button class="start-button" id="startButton" style="font-family: 'Nunito', sans-serif; background-color: darkgreen;">Start Meditation</button>
    <button id="pauseButton" style="display: none; opacity: 0; position: fixed; left: 50%; top: 50%; transform: translate(-50%, -50%); background: darkgreen; border: none; padding: 20px; border-radius: 10px; cursor: pointer; transition: opacity 0.5s;">
    ...
    <div id="progressContainer" style="display: none; position: fixed; left: 50%; top: 55%; transform: translateX(-50%); width: 80%; height: 50px; background-color: #ddd; border-radius: 5px; cursor: pointer;">
        <div id="progressBar" style="height: 100%; background-color: darkgreen; width: 0%; border-radius: 5px; transition: width 0.2s ease-in-out;"></div>
    </div>
        ...
        <div class="modal-content">
            ...
            <h3>You’ve earned <strong>
                <span class="blossoms-number">
                    <?php echo ($currentLevel <= 1) ? 10 : 5; ?>
                </span>
                </strong> Blossoms.</h3>
            <h3>You’ve also gained <strong>
            <span class="xp-number">
                <?php echo max(4000 - ($currentLevel * 1000), 500); ?>
            </span>
                </strong> XP.</h3>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const selfConfidenceImage = document.getElementById("selfConfidenceImage");
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function updateBlossomsAndXP() {
            fetch('selfconfidence_boost.php', {
                method: 'POST',
                headers: {
